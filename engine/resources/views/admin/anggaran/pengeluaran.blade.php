    ...
    <div class="row">
        <div class="col-md-8">
            <div class="header-name">
                <h1 class="tour-step-one">Pengeluaran</h1>
                <p>Manajemen Pengeluaran</p>
            </div>
        </div>
        <div class="col-md-4 text-right">
            <button class="btn btn-success pull-right mt-3 btnAdd" type="button"><i class="fas fa-plus"></i> Tambah Pengeluaran</button>
        </div>
    </div>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    ...
                                    <form action="{{route('anggaran.pengeluaran.destroy',$item->id)}}"
        ...
        <div class="modal-dialog modal-lg" role="document">
            ...
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title" id="titleUpdate">Tambah Pengeluaran</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span
                            aria-hidden="true">&times;</span></button>
                </div>
                <div class="modal-body">

                    <form action="{{route('anggaran.pengeluaran.store')}}" id="frmAdd" method="POST"
...
<script>
    $(document).ready(function () {
        ...
        $('.btnAdd').on('click', function (e) {
            $('#frmAdd').attr('action',"{{route('anggaran.pengeluaran.store')}}");
            $('#frmAdd').append('<div id="method"></div>');
            $('#method').html('{{ method_field("POST") }}');
            $('#titleUpdate').html("Tambah Pengeluaran");
            $('#created_at').val('');
        ...
        $('.btnEdit').on('click', function (e) {
            var id = $(this).data('id');
                $('#frmAdd').attr('action','{{ url("admin/anggaran/pengeluaran") }}/'+id);
                $('#frmAdd').append('<div id="method"></div>');
                $('#method').html('{{ method_field("PATCH") }}');
                $.ajax({
                    type: "get",
                    url: "{{ url('admin/anggaran/pengeluaran') }}/"+id,
                ...
                $('#titleUpdate').html("Edit Pengeluaran");
                $('#createLaporanModal').modal('show');
        });
